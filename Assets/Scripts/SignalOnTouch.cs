...
public class SignalOnTouch : MonoBehaviour
{
    ...
    //when we enter a triger area, call SendSignal.
    ...
    //Check to see if this object was tagged as player,
    //and invoke the UnityEvent if it was.
    void SendSignal(GameObject objectThatHit)
    {
        //Was this object tagged Player?
        if (objectThatHit.CompareTag("Player"))
        {
            //If we have an AudioSound, attempt to play it
            if (playAudioOnTouch) 
            {
                var audio = GetComponent<AudioSource>();

                //If we have an audio component, and this component's parent are active,
                //then play
